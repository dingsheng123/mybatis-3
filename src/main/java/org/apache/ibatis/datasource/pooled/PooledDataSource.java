...
public class PooledDataSource implements DataSource {
     ...
     * 可以被设置为和数据库连接超时时间一样，来避免不必要的侦测，默认值：0
     * 即所有连接每一时刻都被侦测 — 仅当 poolPingEnabled 为 true 时适用
     */
    ...
    protected void pushConnection(PooledConnection conn) throws SQLException {
        synchronized (state) {
            ...
            //通过ping来检测连接是否有效
            if (conn.isValid()) {
                //连接有效，判断空闲连接数是否小于最大空闲连接数，并且判断该连接是否和当前连接池匹配
                if (state.idleConnections.size() < poolMaximumIdleConnections
                    && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    //统计连接使用时长
                    state.accumulatedCheckoutTime += conn.getCheckoutTime();
                    //如果该连接非自动提交的，需要进行回滚。即将原有执行中的事务，全部回滚。
    ...
    private PooledConnection popConnection(String username, String password) throws SQLException {
        ...
        //当connect连接为null时，循环获取
        while (conn == null) {
            synchronized (state) {
                if (!state.idleConnections.isEmpty()) {
                    // Pool has available connection
                    //有可用的空闲连接，通过移除的方式获取首个空闲连接
                    ...
                    if (state.activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        // 不能创建新的连接，先获取最初的时候第一个创建的活跃connection连接
                        ...
                        if (longestCheckoutTime > poolMaximumCheckoutTime) {
                            ...
                            //如果不是自动提交的，需要进行回滚，将原有执行中的所有事务都进行回滚
                            ...
                            try {
                                ...
                                //等待，直到超时，或者pingConnection方法中归还连接时唤醒
